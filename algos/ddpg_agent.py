...
class DDPGAgent(BaseAgent):
    def __init__(self, config=None):
        ...
        self.buffer = ReplayBuffer((self.observation_space_dim, ), self.action_dim, max_size=int(float(1e6)))
    ...
    def calculate_critic_loss(self, current_Q, target_Q):
        ########## Your code starts here. ##########
        critic_loss = nn.HuberLoss()(current_Q, target_Q)
        ########## Your code ends here. ##########
        return critic_loss
    ...
    def get_action(self, observation, evaluation=False):
        ...
        expl_noise = 0.1 * self.max_action  # Stddev of the exploration noise
        action = self.pi(x)
        
        if not evaluation:
            noise = torch.normal(0, expl_noise, size=(self.action_dim,), device=self.device)  # Create noise as a PyTorch tensor
            action = action + noise
        
        action = action.squeeze()  # Remove unnecessary dimensions
        return action, {}  # Convert to NumPy array for the environment
